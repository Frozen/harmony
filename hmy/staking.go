...
func (hmy *Harmony) SendStakingTx(ctx context.Context, signedStakingTx *staking.StakingTransaction) error {
	stx, _, _, _ := rawdb.ReadStakingTransaction(hmy.BlockChain().ChainDb(), signedStakingTx.Hash())
	if stx == nil {
...
func (hmy *Harmony) GetValidatorInformation(
	...
	lastBlockOfEpoch := shard.Schedule.EpochLastBlock(hmy.consensus.EpochChain().CurrentBlock().Header().Epoch().Uint64())

	computed.BlocksLeftInEpoch = lastBlockOfEpoch - hmy.consensus.EpochChain().CurrentBlock().Header().Number().Uint64()
	...
	if defaultReply.CurrentlyInCommittee {
		defaultReply.Performance = &staking.CurrentEpochPerformance{
			...
			Epoch:                    hmy.consensus.EpochChain().CurrentBlock().Header().Epoch().Uint64(),
			Block:                    hmy.consensus.EpochChain().CurrentBlock().Header().Number().Uint64(),
		}
	}
