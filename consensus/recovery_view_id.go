...
func (consensus *Consensus) InitializeEmergencyRecoveryLeader() error {
	...
	if blockchain == nil || blockchain.CurrentHeader() == nil {
		return errors.New("cannot initialize emergency recovery leader without blockchain head")
	}
	...
	if err != nil {
		return fmt.Errorf("read recovery shard state: %w", err)
	}
	committee, err := shardState.FindCommitteeByID(consensus.ShardID)
	if err != nil {
		return fmt.Errorf("find recovery committee: %w", err)
	}
	leader := consensus.current.getNextLeaderKey(
		blockchain, consensus.decider(), viewID, committee,
	)
	if leader == nil {
		return errors.New("cannot derive emergency recovery leader")
	}
